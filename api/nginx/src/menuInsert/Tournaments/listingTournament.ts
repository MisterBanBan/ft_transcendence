...
export const tournamentList = () => {
    ...
    const sortedTournament = tournament.filter(f => typeof f.trophee === 'number').sort((a,b) => b.trophee - a.trophee);
    let nb = 0;
    return `
        <div class="h-full w-full overflow-y-auto ">
            ${sortedTournament.map(f => `
            <div class="flex flex-row justify-between items-center gap-4 responsive-text-historique ">
                ...
                <button  class="tournament-btn responsive-text-historique">${f.name}</button>
