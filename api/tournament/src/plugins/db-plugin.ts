...
export default async function (server: FastifyInstance, opts: any) {
    ...
    let db: Database;
    try {
        db = await open({ filename: "./database/tournament/tournament_db.sqlite", driver: sqlite3.Database });
    ...
    const umzug = new Umzug({
        ...
        context: db,
        storage: new JSONStorage({ path: "./database/tournament/migrations.json" })
    });
    await umzug.up();
